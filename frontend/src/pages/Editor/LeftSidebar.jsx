...
export function LeftSidebar({ activeTab, setActiveTab, project, updateSlide, videoCategory, primaryColor, selectedSlideId, setSelectedSlideId, selectedVoice, setSelectedVoice }) {
  ...
  return (
    <div className="w-80 bg-[#0a0f1a] border-r border-white/[0.06] flex flex-col h-full">
      <div className="flex border-b border-white/[0.06] p-1 gap-0.5 overflow-x-auto scrollbar-hide bg-[#060a14]">
        {TABS.map(tab => {
          const IconComp = TAB_ICONS[tab.icon];
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-col items-center gap-0.5 px-2.5 py-2 rounded-sm flex-shrink-0 transition ${activeTab === tab.id ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.04]'}`} data-testid={`tab-${tab.id}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        {activeTab === 'script' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Slides</h3>
              <span className="text-[10px] text-slate-400">{project?.slides?.length || 0} total</span>
            </div>
            {project?.slides?.map((slide, idx) => (
              <div key={slide.id} onClick={() => setSelectedSlideId(slide.id)} className={`p-3 rounded-md border-2 cursor-pointer group transition ${selectedSlideId === slide.id ? 'border-indigo-500 bg-indigo-500/5' : 'border-white/[0.08] bg-[#0d1117] hover:border-indigo-500/30'}`}>
                <div className="flex items-start gap-3">
                  <div className="w-16 h-10 rounded-sm bg-[#111827] flex-shrink-0 overflow-hidden flex items-center justify-center border border-white/[0.08]">
                    {slide.assetUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-xs font-bold text-slate-200 truncate">{slide.title}</p>
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[9px] text-slate-400 font-semibold">
                      ...
                      {slide.titlePosition && slide.titlePosition !== 'hidden' && <span className="text-amber-400">· Title: {slide.titlePosition}</span>}
                    </div>
                  </div>
                </div>
                {selectedSlideId === slide.id && (
                  <div className="mt-3 pt-3 border-t border-white/[0.08] space-y-2" onClick={(e) => e.stopPropagation()}>
                    <div>
                      <label className="text-[9px] text-slate-500 font-bold uppercase">Title Text</label>
                      <input type="text" value={slide.title || ''} onChange={(e) => updateSlide(slide.id, { title: e.target.value })} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1.5 text-xs text-slate-200 focus:border-indigo-500 outline-none mt-0.5" data-testid={`slide-title-input-${idx}`} />
                    </div>
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-1 mt-1">
                        {[
                          ...
                          <button key={pos.id} onClick={() => updateSlide(slide.id, { titlePosition: pos.id })} className={`py-1 rounded-none text-[8px] font-bold border-2 transition ${(slide.titlePosition || 'top-left') === pos.id ? 'bg-amber-500 text-white border-amber-500' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid={`title-pos-${pos.id}-${idx}`}>{pos.label}</button>
                        ))}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <label className="text-[9px] text-slate-500 font-bold uppercase">Show Title</label>
                      <button onClick={() => updateSlide(slide.id, { titlePosition: slide.titlePosition === 'hidden' ? 'top-left' : 'hidden' })} className={`px-2 py-0.5 rounded-none text-[9px] font-bold border-2 transition ${slide.titlePosition === 'hidden' ? 'bg-[#111827] text-slate-400 border-white/[0.08]' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`} data-testid={`title-toggle-${idx}`}>{slide.titlePosition === 'hidden' ? 'Hidden' : 'Visible'}</button>
                    </div>
                    <div>
                      <label className="text-[9px] text-slate-500 font-bold uppercase">Narration</label>
                      <textarea value={slide.narration || ''} onChange={(e) => updateSlide(slide.id, { narration: e.target.value })} rows={2} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1.5 text-[10px] text-slate-200 focus:border-indigo-500 outline-none mt-0.5 resize-none" />
                    </div>
                    <div className="flex gap-2">
                      <label className="flex-1">
                        <span className="text-[9px] text-slate-500 font-bold">Duration (s)</span>
                        <input type="number" min={1} max={30} step={1} value={slide.duration || 6} onChange={(e) => updateSlide(slide.id, { duration: parseInt(e.target.value) || 6 })} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" />
                      </label>
                      <label className="flex-1">
                        <span className="text-[9px] text-slate-500 font-bold">Transition</span>
                        <select value={slide.transition || 'fade'} onChange={(e) => updateSlide(slide.id, { transition: e.target.value })} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none">
        ...
        {activeTab === 'assets' && (
          <div className="space-y-4">
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Upload Assets</h3>
              <div className="grid grid-cols-3 gap-2">
                <label className="flex flex-col items-center gap-1 p-3 rounded-md border-2 border-dashed border-white/[0.08] hover:border-indigo-500/40 cursor-pointer hover:bg-indigo-500/5 transition">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Slide Assets</h3>
                <button onClick={generateAllMissing} className="flex items-center gap-1 text-[10px] font-bold text-indigo-400 hover:text-indigo-300 px-2 py-1 rounded-sm border border-indigo-500/20 bg-indigo-500/10 hover:bg-indigo-100 transition">
              ...
              <div className="space-y-3">
                {project?.slides?.map((slide, idx) => (
                  <div key={slide.id} className={`p-3 rounded-md border-2 ${selectedSlideId === slide.id ? 'border-indigo-500 bg-indigo-500/5' : 'border-white/[0.08] bg-[#0d1117]'}`}>
                    <div className="flex gap-3">
                      <div className="w-20 h-14 rounded-sm bg-[#111827] flex-shrink-0 overflow-hidden flex items-center justify-center border border-white/[0.08]">
                        {slide.assetUrl ? (
                          <img src={slide.assetUrl.startsWith('/api') ? `${process.env.REACT_APP_BACKEND_URL}${slide.assetUrl}` : slide.assetUrl} className="w-full h-full object-cover" alt="" />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-slate-200 mb-1">Slide {idx + 1}: {slide.title}</p>
                        <textarea
                          ...
                          className="w-full text-[9px] text-slate-600 bg-[#0d1117] border-2 border-white/[0.08] rounded-sm p-1.5 resize-none h-12 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none"
                        ...
                        <div className="flex gap-1.5 mt-1.5">
                          ...
                          <label className="flex items-center gap-1 px-2 py-1 rounded-none text-[9px] font-bold border-2 border-white/[0.08] text-slate-600 cursor-pointer hover:bg-[#0d1117]">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {suggestedAssets.map(asset => (
                    <button key={asset.id} onClick={() => { if (selectedSlideId) updateSlide(selectedSlideId, { assetType: asset.type, assetUrl: asset.url }); }} className="group relative rounded-sm overflow-hidden bg-[#111827] aspect-square ring-2 ring-amber-300 hover:ring-amber-500 transition" title={`${asset.prompt} (${asset.score} keyword matches)`} data-testid={`suggested-asset-${asset.id}`}>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">My Library</h3>
                <button onClick={loadUserLibrary} className="text-[9px] text-indigo-400 hover:text-indigo-300" data-testid="refresh-library">
              ...
              <div className="flex gap-1 mb-2 flex-wrap">
                {[{ id: 'all', label: 'All' }, { id: 'image', label: 'Images' }, { id: 'voice', label: 'Voices' }, { id: 'video', label: 'Videos' }, { id: 'audio', label: 'Audio' }].map(lc => (
                  <button key={lc.id} onClick={() => setLibraryFilter(lc.id)} className={`px-2 py-0.5 rounded-none text-[9px] font-bold border-2 transition ${libraryFilter === lc.id ? 'bg-indigo-500/10 text-indigo-400 border-indigo-300' : 'bg-[#0d1117] text-slate-400 border-white/[0.08] hover:border-slate-300'}`} data-testid={`editor-library-filter-${lc.id}`}>
                    {lc.label}
                  </button>
                ))}
              </div>
              {libraryLoading ? (
                ...
                <div className="grid grid-cols-3 gap-1.5 max-h-[200px] overflow-y-auto scrollbar-hide">
                  {userLibrary.map(asset => (
                    <button key={asset.id} onClick={() => { if (selectedSlideId && (asset.type === 'image' || asset.type === 'video')) updateSlide(selectedSlideId, { assetType: asset.type, assetUrl: asset.url }); }} className="group relative rounded-sm overflow-hidden bg-[#111827] aspect-square hover:ring-2 hover:ring-indigo-500 transition border border-white/[0.08]" title={asset.prompt || 'No description'} data-testid={`library-item-${asset.id}`}>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 px-1 py-0.5 bg-[#0d1117]/80 text-[7px] text-slate-500 truncate font-bold">{asset.type}</div>
                    </button>
                  ))}
                </div>
              )}
        ...
        {activeTab === 'music' && (
          <div className="space-y-4">
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Active Music Tracks</h3>
              
              {project?.bgmUrl && (
                <div className="p-3 rounded-md bg-indigo-500/10 border-2 border-indigo-500/20 space-y-2 mb-2">
                  <div className="flex items-center gap-2">
                    <Music className="w-3 h-3 text-indigo-500" />
                    <span className="text-[10px] text-indigo-300 font-bold truncate flex-1">Primary: {project.bgmName || 'Custom Track'}</span>
                    <span className="text-[8px] bg-indigo-200 text-indigo-300 px-1.5 py-0.5 rounded font-bold">LOOPS</span>
              ...
              {(project?.musicTracks || []).map((track, idx) => (
                <div key={idx} className="p-3 rounded-md bg-emerald-500/10 border-2 border-emerald-200 space-y-2 mb-2">
                  <div className="flex items-center gap-2">
              ...
              <label className="flex flex-col items-center gap-2 p-3 border-2 border-dashed border-white/[0.08] rounded-md cursor-pointer hover:border-indigo-500/40 hover:bg-indigo-500/5 transition" data-testid="upload-bgm-label">
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Music Library <span className="text-[9px] text-slate-400 font-normal ml-1">{musicLibrary.total || 0} tracks</span></h3>
              <div className="flex flex-wrap gap-1 mb-3">
                <button onClick={() => setMusicFilter('all')} className={`px-2 py-0.5 rounded-none text-[9px] font-bold border-2 transition ${musicFilter === 'all' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-300' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid="music-filter-all">All</button>
                {musicLibrary.categories?.map(c => (
                  <button key={c} onClick={() => setMusicFilter(c)} className={`px-2 py-0.5 rounded-none text-[9px] font-bold capitalize border-2 transition ${musicFilter === c ? 'bg-indigo-500/10 text-indigo-400 border-indigo-300' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid={`music-filter-${c}`}>{c}</button>
                ))}
              </div>
              <div className="space-y-1 max-h-[280px] overflow-y-auto pr-1">
                {(musicFilter === 'all' ? musicLibrary.tracks : musicLibrary.tracks?.filter(t => t.category === musicFilter))?.map(track => {
                  ...
                  return (
                  <div key={track.id} className={`flex items-center gap-2 p-2 rounded-sm border-2 transition cursor-pointer group ${isActive ? 'bg-indigo-500/10 border-indigo-300' : 'bg-[#0d1117] border-white/[0.08] hover:border-slate-300'}`} data-testid={`music-track-${track.id}`}>
                    <button onClick={() => { if (previewingTrack === track.id) { previewAudioRef.current?.pause(); setPreviewingTrack(null); } else { if (previewAudioRef.current) { previewAudioRef.current.src = `${API.replace('/api', '')}${track.url}`; previewAudioRef.current.play().catch(() => {}); } setPreviewingTrack(track.id); } }} className="w-6 h-6 rounded-sm bg-[#111827] group-hover:bg-indigo-500 flex items-center justify-center flex-shrink-0 transition">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-slate-200 font-bold truncate">{track.name}</p>
                    ...
                    }} className={`px-2 py-0.5 rounded-none text-[9px] font-bold flex-shrink-0 transition ${isActive ? 'bg-indigo-500 text-white' : 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 hover:bg-indigo-100'}`}>
                      {isActive ? 'Active' : 'Add'}
                    </button>
                  </div>
                  );
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Sound Effects <span className="text-[9px] text-slate-400 font-normal ml-1">{sfxLibrary.total || 0} sounds</span></h3>
              {selectedSlideId && project?.slides?.find(s => s.id === selectedSlideId)?.sfxUrl && (
                <div className="flex items-center gap-2 p-2 mb-2 rounded-sm bg-pink-500/10 border-2 border-pink-500/20">
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                <button onClick={() => setSfxFilter('all')} className={`px-2 py-0.5 rounded-none text-[9px] font-bold border-2 transition ${sfxFilter === 'all' ? 'bg-pink-500/10 text-pink-400 border-pink-300' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid="sfx-filter-all">All</button>
                {sfxLibrary.categories?.map(c => (
                  <button key={c} onClick={() => setSfxFilter(c)} className={`px-2 py-0.5 rounded-none text-[9px] font-bold capitalize border-2 transition ${sfxFilter === c ? 'bg-pink-500/10 text-pink-400 border-pink-300' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid={`sfx-filter-${c}`}>{c}</button>
                ))}
              </div>
              <div className="space-y-1 max-h-[250px] overflow-y-auto pr-1">
                {(sfxFilter === 'all' ? sfxLibrary.sounds : sfxLibrary.sounds?.filter(s => s.category === sfxFilter))?.map(sfx => (
                  <div key={sfx.id} className={`flex items-center gap-2 p-2 rounded-sm border-2 transition group ${project?.slides?.find(s => s.id === selectedSlideId)?.sfxUrl === sfx.url ? 'bg-pink-500/10 border-pink-300' : 'bg-[#0d1117] border-white/[0.08] hover:border-slate-300'}`} data-testid={`sfx-item-${sfx.id}`}>
                    <button onClick={() => { if (previewingTrack === sfx.id) { previewAudioRef.current?.pause(); setPreviewingTrack(null); } else { if (previewAudioRef.current) { previewAudioRef.current.src = `${API.replace('/api', '')}${sfx.url}`; previewAudioRef.current.play().catch(() => {}); } setPreviewingTrack(sfx.id); } }} className="w-6 h-6 rounded-sm bg-[#111827] group-hover:bg-pink-500/100 flex items-center justify-center flex-shrink-0 transition">
        ...
        {activeTab === 'voice' && (
          <div className="space-y-4">
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Select Voice (for all slides)</h3>
              <select value={selectedVoice} onChange={(e) => setSelectedVoice(e.target.value)} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-3 py-2.5 text-sm text-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 outline-none">
                {VOICES.map(voice => (
            ...
            <div className="p-4 rounded-md border-2 border-indigo-500/20 bg-indigo-500/5">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-sm font-bold text-slate-200">Generate All Voices</p>
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Slide Narrations</h3>
              <div className="space-y-2 max-h-[300px] overflow-y-auto scrollbar-hide">
                {project?.slides?.map((slide, idx) => (
                  <div key={slide.id} className={`p-2.5 rounded-md border-2 ${slide.voiceUrl ? 'border-emerald-500/20 bg-emerald-500/10' : 'border-white/[0.08] bg-[#0d1117]'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold text-slate-600">Slide {idx + 1}</span>
                      {slide.voiceUrl ? (
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => playAudio(slide.voiceUrl)} className="p-1 rounded-sm bg-emerald-100 text-emerald-400 hover:bg-emerald-200">
                            {playingAudio === slide.voiceUrl ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3" />}
                          </button>
                          <span className="text-[9px] text-emerald-400 font-bold"><Check className="w-3 h-3 inline" /> Done</span>
                        </div>
                      ) : (
        ...
        {activeTab === 'captions' && (
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Caption Style</h3>
            <div className="grid grid-cols-2 gap-2">
              {CAPTION_STYLES.map(style => (
                <button key={style.id} onClick={() => setActiveCaptionStyleId(style.id)} className={`p-3 rounded-md border-2 text-center transition ${activeCaptionStyleId === style.id ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/[0.08] hover:border-slate-300'}`} data-testid={`caption-${style.id}`}>
            ...
            <div>
              ...
              <div className="flex gap-1">
                {['sentence', 'lines', 'words'].map(m => (
                  <button key={m} onClick={() => setCaptionMode(m)} className={`flex-1 py-1.5 rounded-none text-xs font-bold border-2 transition capitalize ${captionMode === m ? 'bg-indigo-500 text-white border-indigo-600' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid={`caption-mode-${m}`}>{m}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1">
                {[{ id: 'top', label: 'Top' }, { id: 'center', label: 'Center' }, { id: 'bottom', label: 'Bottom' }].map(pos => (
                  <button key={pos.id} onClick={() => setCaptionPosition(pos.id)} className={`py-1.5 rounded-none text-[10px] font-bold border-2 transition ${captionPosition === pos.id ? 'bg-indigo-500 text-white border-indigo-600' : 'bg-[#0d1117] text-slate-400 border-white/[0.08]'}`} data-testid={`caption-pos-${pos.id}`}>{pos.label}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <select value={captionFont} onChange={(e) => setCaptionFont(e.target.value)} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-3 py-2 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid="caption-font-select">
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <input type="color" value={captionColor || '#ffffff'} onChange={(e) => setCaptionColor(e.target.value)} className="w-7 h-7 rounded-sm border-2 border-white/[0.08] cursor-pointer p-0.5" data-testid="caption-text-color" />
                  <input type="text" value={captionColor} onChange={(e) => setCaptionColor(e.target.value)} placeholder="Auto" className="flex-1 bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-[10px] text-slate-200 focus:border-indigo-500 outline-none" />
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="color" value={captionBgColor || '#000000'} onChange={(e) => setCaptionBgColor(e.target.value)} className="w-7 h-7 rounded-sm border-2 border-white/[0.08] cursor-pointer p-0.5" data-testid="caption-bg-color" />
                  <input type="text" value={captionBgColor} onChange={(e) => setCaptionBgColor(e.target.value)} placeholder="Auto" className="flex-1 bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-[10px] text-slate-200 focus:border-indigo-500 outline-none" />
            ...
            <div className="p-3 rounded-md bg-[#0d1117] border-2 border-white/[0.08]">
        ...
        {activeTab === 'graphics' && activeSlide && (
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Motion Graphics</h3>
            ...
            {(activeSlide.graphics || []).length > 0 && (
              <div className="space-y-2">
                ...
                {activeSlide.graphics.map((g, gi) => (
                  <div key={gi} className="flex items-center gap-2 p-2 rounded-sm bg-indigo-500/10 border-2 border-indigo-500/20">
                    <span className="text-[10px] text-indigo-400 font-bold capitalize flex-1">{g.type?.replace('-', ' ')}</span>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                }} className="p-3 rounded-md border-2 border-white/[0.08] hover:border-indigo-500/40 hover:bg-indigo-500/5 transition text-left" data-testid={`add-graphic-${opt.type}`}>
                  <p className="text-[11px] text-slate-200 font-bold">{opt.label}</p>
            ...
            {(activeSlide.graphics || []).length > 0 && (
              <div className="space-y-3">
                ...
                {activeSlide.graphics.map((g, gi) => (
                  <div key={gi} className="p-2 rounded-md bg-[#0d1117] border-2 border-white/[0.08] space-y-2">
                    <p className="text-[10px] text-indigo-400 font-bold capitalize">{g.type?.replace('-', ' ')} #{gi + 1}</p>
                    <div className="flex gap-2">
                      <label className="flex-1">
                        <span className="text-[9px] text-slate-400 font-bold">Start (s)</span>
                        <input type="number" min={0} step={0.5} value={g.startTime || 0} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], startTime: parseFloat(e.target.value) || 0 }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-start`} />
                      </label>
                      <label className="flex-1">
                        <span className="text-[9px] text-slate-400 font-bold">Duration (s)</span>
                        <input type="number" min={0.5} step={0.5} value={g.duration || 3} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], duration: parseFloat(e.target.value) || 3 }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-duration`} />
                      </label>
                    </div>
                    {(g.type === 'title-card' || g.type === 'kinetic-text') && (
                      <label>
                        <span className="text-[9px] text-slate-400 font-bold">{g.type === 'title-card' ? 'Title' : 'Text'}</span>
                        <input type="text" value={g.title || g.text || ''} onChange={(e) => { const updated = [...activeSlide.graphics]; const key = g.type === 'title-card' ? 'title' : 'text'; updated[gi] = { ...updated[gi], [key]: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-text`} />
                      </label>
                    )}
                    {g.type === 'title-card' && (
                      <label>
                        <span className="text-[9px] text-slate-400 font-bold">Subtitle</span>
                        <input type="text" value={g.subtitle || ''} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], subtitle: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-subtitle`} />
                      </label>
                    )}
                    {g.type === 'lower-third' && (
                      <>
                        <label>
                          ...
                          <input type="text" value={g.name || ''} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], name: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-name`} />
                        </label>
                        <label>
                          <span className="text-[9px] text-slate-400 font-bold">Title</span>
                          <input type="text" value={g.title || ''} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], title: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-title`} />
                        </label>
                      </>
                    )}
                    {g.type === 'stat-counter' && (
                      <>
                        <div className="flex gap-2">
                          <label className="flex-1">
                            ...
                            <input type="text" value={g.value || ''} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], value: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-value`} />
                          </label>
                          <label className="flex-1">
                            <span className="text-[9px] text-slate-400 font-bold">Label</span>
                            <input type="text" value={g.label || ''} onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], label: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" data-testid={`graphic-${gi}-label`} />
                          </label>
                        </div>
                        <div className="flex gap-2">
                          <label className="flex-1">
                            <span className="text-[9px] text-slate-400 font-bold">Prefix</span>
                            <input type="text" value={g.prefix || ''} placeholder="$" onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], prefix: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" />
                          </label>
                          <label className="flex-1">
                            <span className="text-[9px] text-slate-400 font-bold">Suffix</span>
                            <input type="text" value={g.suffix || ''} placeholder="%" onChange={(e) => { const updated = [...activeSlide.graphics]; updated[gi] = { ...updated[gi], suffix: e.target.value }; updateSlide(activeSlide.id, { graphics: updated }); }} className="w-full bg-[#0d1117] border-2 border-white/[0.08] rounded-sm px-2 py-1 text-xs text-slate-200 focus:border-indigo-500 outline-none" />
                          </label>
                        </div>
                      </>
                    )}
        ...
        {activeTab === 'effects' && activeSlide && (
          <div className="space-y-4">
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Transition</h3>
              <div className="grid grid-cols-2 gap-2">
                {TRANSITION_OPTIONS.map(trans => (
                  <button key={trans} onClick={() => updateSlide(activeSlide.id, { transition: trans })} className={`p-2.5 rounded-md border-2 text-xs font-bold capitalize transition ${activeSlide.transition === trans ? 'border-indigo-500 bg-indigo-500/10 text-indigo-300' : 'border-white/[0.08] text-slate-500 hover:border-slate-300'}`} data-testid={`trans-${trans}`}>
                    {trans}
                  </button>
                ))}
              </div>
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Visual Effects</h3>
              <div className="grid grid-cols-2 gap-2">
                {VFX_OPTIONS.map(vfx => (
                  <button key={vfx} onClick={() => updateSlide(activeSlide.id, { vfx })} className={`p-2.5 rounded-md border-2 text-xs font-bold capitalize transition ${activeSlide.vfx === vfx ? 'border-indigo-500 bg-indigo-500/10 text-indigo-300' : 'border-white/[0.08] text-slate-500 hover:border-slate-300'}`} data-testid={`vfx-${vfx}`}>
                    {vfx}
                  </button>
                ))}
              </div>
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-2">Batch Apply</h3>
              <div className="space-y-2">
                <button onClick={() => project?.slides?.forEach(s => updateSlide(s.id, { transition: activeSlide.transition }))} className="w-full px-3 py-2 rounded-none text-[10px] font-bold border-2 border-white/[0.08] text-slate-600 hover:border-indigo-500/30 hover:bg-indigo-500/10 transition">
                  Apply transition to all slides
                </button>
                <button onClick={() => project?.slides?.forEach(s => updateSlide(s.id, { vfx: activeSlide.vfx }))} className="w-full px-3 py-2 rounded-none text-[10px] font-bold border-2 border-white/[0.08] text-slate-600 hover:border-indigo-500/30 hover:bg-indigo-500/10 transition">
